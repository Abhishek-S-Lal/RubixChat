package com.abhishekslal.chat.android.Friends;
...
import com.abhishekslal.chat.android.Model.Requests;
...
public class FriendRequestsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        context = this;
        toolbar = findViewById(R.id.friend_req_appbar);
        ...
        //for friend request
        request_list = findViewById(R.id.friendRequestList);
    ...
    public void onStart() {
        ...
        FirebaseRecyclerAdapter<Requests, RequestsViewHolder> adapter = new FirebaseRecyclerAdapter<Requests, RequestsViewHolder>(recyclerOptions) {
            @Override
                                                ...
                                                builder.setItems(options, new DialogInterface.OnClickListener() {
                                                    ...
                                                    public void onClick(DialogInterface dialog, int which) {
                                                        ...
                                                        if (which == 0){
                                                            ...
                                                            friendsDatabaseReference.child(current_user_id).child(userID).child("date").setValue(friendshipDate)
                                                                    .addOnCompleteListener(new OnCompleteListener<Void>() {
                                                                        @Override
                                                                        public void onComplete(@NonNull Task<Void> task) {

                                                                            friendsDatabaseReference.child(userID).child(current_user_id).child("date").setValue(friendshipDate)
                                                                                    .addOnCompleteListener(new OnCompleteListener<Void>() {
                                                                                        @Override
                                                                                        public void onComplete(@NonNull Task<Void> task) {
                                                                                            /**
                                                                                             ...
                                                                                             */
                                                                                            friendReqDatabaseReference.child(current_user_id).child(userID).removeValue()
                                                                                                    .addOnCompleteListener(new OnCompleteListener<Void>() {
                                                                                                        @Override
                                                                                                        public void onComplete(@NonNull Task<Void> task) {
                                                                                                            if (task.isSuccessful()){
                                                                                                                // delete from users friend_requests node, receiver >> sender > values
                                                                                                                friendReqDatabaseReference.child(userID).child(current_user_id).removeValue()
                                                                                                                        .addOnCompleteListener(new OnCompleteListener<Void>() {
                                                                                                                            @Override
                                                                                                                            public void onComplete(@NonNull Task<Void> task) {
                                                                                                                                if (task.isSuccessful()){
                                                                                                                                    ...
                                                                                                                                    Snackbar snackbar = Snackbar
                                                                                                                                            .make(getWindow().getDecorView().getRootView(), "This person is now your friend", Snackbar.LENGTH_LONG);
                                                                                                                                    // Changing message text color
                                                                                                                                    View sView = snackbar.getView();
                                                                                                                                    sView.setBackgroundColor(ContextCompat.getColor(context, R.color.colorPrimary));
                                                                                        ...
                                                                                        .addOnCompleteListener(new OnCompleteListener<Void>() {
                                                                                            ...
                                                                                            public void onComplete(@NonNull Task<Void> task) {
                                                                                                if (task.isSuccessful()){
                                                                                                    ...
                                                                                                    Snackbar snackbar = Snackbar
                                                                                                            .make(getWindow().getDecorView().getRootView(), "Canceled Request", Snackbar.LENGTH_LONG);
                                                                                                    // Changing message text color
                                                                                                    View sView = snackbar.getView();
                                                                                                    sView.setBackgroundColor(ContextCompat.getColor(context, R.color.colorPrimary));
                                                                                                    TextView textView = sView.findViewById(R.id.snackbar_text);
                                                                                ...
                                                                                friendReqDatabaseReference.child(userID).child(current_user_id).removeValue()
                                                                                        .addOnCompleteListener(new OnCompleteListener<Void>() {
                                                                                            ...
                                                                                            public void onComplete(@NonNull Task<Void> task) {
                                                                                                if (task.isSuccessful()){
                                                                                                    Snackbar snackbar = Snackbar
                                                                                                            .make(getWindow().getDecorView().getRootView(), "Cancel Sent Request", Snackbar.LENGTH_LONG);
                                                                                                    // Changing message text color
                                                                                                    View sView = snackbar.getView();
                                                                                                    sView.setBackgroundColor(ContextCompat.getColor(context, R.color.colorPrimary));
                                                                                                    TextView textView = sView.findViewById(R.id.snackbar_text);
