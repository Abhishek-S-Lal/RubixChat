package com.abhishekslal.chat.android.Profile;
...
import com.abhishekslal.chat.android.Settings.UserProfileEditActivity;
...
public class ProfileActivity extends AppCompatActivity {
    ...
    private DatabaseReference userChatsDatabaseReference;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        userChatsDatabaseReference = FirebaseDatabase.getInstance().getReference().child("userChats");
        ...
        if (!senderID.equals(receiver_userID)) { // condition for current owner / sender id
            sendFriendRequest_Button.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (CURRENT_STATE.equals("not_friends")) {
                        ...
                        builder.setMessage("Your conversation in the chats will also be deleted mutually");
                        builder.setTitle("Do you really want to UnFriend?");
            ...
            go_my_profile.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Intent intent = new Intent(ProfileActivity.this, UserProfileEditActivity.class);
    ...
    private void deleteConversation() {
        //find conversation id and delete conversation from userChats and messages
        final String chat_id;
        if (senderID.compareTo(receiver_userID) > 0) {
            chat_id = senderID + "_" + receiver_userID;
        } else {
            chat_id = receiver_userID + "_" + senderID;
        }
        userChatsDatabaseReference.child(senderID).child("private").child(chat_id).removeValue()
                .addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()) {
                            userChatsDatabaseReference.child(receiver_userID).child("private").child(chat_id).removeValue()
                                    .addOnCompleteListener(new OnCompleteListener<Void>() {
                                        @Override
                                        public void onComplete(@NonNull Task<Void> task) {
                                            messagesDatabaseReference.child(chat_id).removeValue()
                                                    .addOnCompleteListener(new OnCompleteListener<Void>() {
                                                        @Override
                                                        public void onComplete(@NonNull Task<Void> task) {
                                                            SweetToast.success(ProfileActivity.this, "Conversation Deleted");
                                                        }
                                                    });
                                        }
                                    });
